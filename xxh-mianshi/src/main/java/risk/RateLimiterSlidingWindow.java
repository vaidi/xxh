...
public class RateLimiterSlidingWindow {
    ...
    public RateLimiterSlidingWindow(int qps){
        ...
        for (int i =0;i <windowArray.length;i++) {
            windowArray[i] = new WindowInfo(curMills,new AtomicInteger(0));
        }
    }
    public synchronized  boolean tryAcquire(){
        long currentTimeMillis = System.currentTimeMillis();
        // 1. 计算当前时间窗口=1000s/10 =100
        long mm = windowSize / windowCount;
        int currentIndex = (int)(currentTimeMillis % windowSize / mm);
        // 2.  更新当前窗口计数 & 重置过期窗口计数
        int sum = 0;
        for (int i = 0; i < windowArray.length; i++) {
